...
## 今後の実装予定機能
### 検索・フィルタリング機能
- [ ] タイトル、著者名、出版社から即座に絞り込む機能
- [ ] 貸出日順（降順・昇順）だけでなく、タイトル順などでソート
- [ ] レビューが空の本を「未読」として抽出する

### 統計・可視化
- [ ] 月ごとの貸出冊数をカウントし、簡易的なグラフで表示
- [ ] 貸出頻度の高い著者をSQLの `GROUP BY` で抽出
- [ ] これまでに借りた本の累計をステータスバーに表示

### データ入出力の強化
- [x] 重複書籍の削除
- [ ] CSVファイルをアプリ上にドロップしてインポート

### UI/UX の改善
- [x] バージョン表示
- [ ] 本をクリックした際、レビューだけでなく ISBN や資料 ID、URL などの全情報を綺麗にレイアウトして表示
- [ ] OSの設定やトグルスイッチに応じて UI カラーを切り替え
- [ ] 右クリックメニューから特定の書影だけ再取得
