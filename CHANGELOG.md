...
## v1.0.0 (2026-04-11)
### 追加
- **ホームディレクトリへのデータ保存機能**: アプリケーション実行パスが散らからないよう、データベースおよびダウンロードした書影の保存先をユーザーのホームディレクトリ（`~/.hon-log/`）に自動生成・統一
- **クリーンな実行ファイル運用**: `.exe` と同じディレクトリにファイルやフォルダを生成しない設計に変更し、デスクトップ等での運用性を向上

### 変更
- **UIファーストの起動フロー**: 起動時に即座にUIを表示し、バックグラウンドで `loans.db` の存在を確認・データ表示を行うように変更
- **堅牢なリソース解決**: `resource_path()` を通じたパス解決により、PyInstaller の `--onefile` モードで固めた際の内蔵リソース（`no-image.png`）読み込みを完全保証
- **ディレクトリ構造の再定義**: データベースを `~/.hon-log/loans.db`、書影を `~/.hon-log/img/` に配置する構成に移行
