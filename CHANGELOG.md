...
## 追加予定の機能
### パフォーマンス改善
- 書影取得の並列化: `concurrent.futures.ThreadPoolExecutor` を導入し、CSVインポート時のISBN取得および画像ダウンロードを並列実行。初回インポート時間を大幅に短縮。
- リクエストの最適化: `requests.Session` をスレッドローカルで再利用し、接続のオーバーヘッドを削減。

### データ管理機能
- 重複データの整理: 同一のISBNを持つ書籍が複数回貸出されている場合、最新の1件のみをグリッドに表示する「重複非表示モード」を実装。
- クレンジング機能: データベース内の特定のレコード、またはキャッシュされた書影ファイルを個別に削除できる管理機能の追加。

### UI/UX の向上
- バージョン情報の表示: アプリケーションのタイトルバーまたは設定画面に現在のバージョン（v1.0.0等）を表示。
- アップデート通知: GitHub API と連携し、新しいリリースがある場合にユーザーへ通知する機能。
