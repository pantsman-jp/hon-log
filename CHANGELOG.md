...
## [v2.3.1](https://github.com/pantsman-jp/hon-log/releases/tag/v2.3.1) (2026-06-15)

### 改善

- 書籍一覧の表示列数をウィンドウ幅に合わせて自動調整するように改善
  - 固定5列表示を廃止し、現在のスクロール領域幅から収まる列数を再計算
  - ウィンドウのリサイズ時は列数が変わった場合のみカードを再配置し、不要なDB再取得を抑制

### 修正

- 書籍カード内で書名が書影に重なって表示される問題を修正
- 統計ウィンドウ内で文字が表示されない不具合の修正

## [v2.3.0](https://github.com/pantsman-jp/hon-log/releases/tag/v2.3.0) (2026-06-15)

### 追加

- メイン画面にタイトル・著者・タグを対象とした検索ボックスを追加
- 一覧の表示件数を確認できる件数表示を追加
- 検索や絞り込み結果が空の場合の空状態メッセージを追加
- 書籍詳細画面から OPAC ページを開けるボタンを追加

### 改善

- アプリ全体の UI デザインを刷新
  - 統一された QSS スタイルを導入し、背景色、ボタン、入力欄、スクロールバー、通知バーの見た目を調整
  - `Fusion` スタイルと日本語フォント設定を適用し、環境差による見た目のばらつきを低減
- メイン画面の操作エリアを改善
  - アプリ名とバージョンを表示するヘッダーを追加
  - CSV取り込み、全削除、統計ボタンにアイコンとツールチップを追加
  - 検索、絞り込み、タグ、並び替え、件数表示を一体化したツールバーに整理
- 書籍一覧の表示をカード型 UI に改善
  - 書影、タイトル、著者、評価、貸出日をカード内に整理して表示
  - 書影だけでなくカード全体をクリックして詳細画面を開けるように変更
  - 長いタイトルや著者名はツールチップで全文を確認できるように調整
- 書籍詳細画面のレイアウトを改善
  - 表紙、タイトル、著者、出版社、巻情報、出版年、ISBN、貸出履歴、評価、タグ、感想欄を読みやすく再配置
  - 感想欄とタグ入力欄にプレースホルダーを追加
  - 閉じるボタンと保存ボタンを整理し、操作の見通しを向上
- 統計画面の見た目を改善
  - グラフタイトル、軸ラベル、グリッド、配色、余白を調整
  - 統計データがない場合に、無反応ではなくメッセージを表示するように変更

### 変更

- タグ保存時に前後の空白を自動で削除するように変更
